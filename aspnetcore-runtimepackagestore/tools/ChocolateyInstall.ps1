...
if (Get-ProcessorBits -eq 64) {
    if (-not (Test-Skip32BitRequested)) {
        if (Test-OsSupports32Bit) {
            ...
            Write-Host 'Installation of 32-bit package store will be skipped because the WOW64 subsystem is not installed.'
        }
    } else {
        Write-Host 'Installation of 32-bit package store will be skipped, as requested by package parameters.'
    }
}
