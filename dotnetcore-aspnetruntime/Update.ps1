...
function global:au_SearchReplace {
    @{
        "$PSScriptRoot\$($Latest.PackageName).nuspec" = @{
            ...
            'id\=\"dotnet\w*-\d+\.\d+-aspnetruntime\"\s+version\=\"[^"]+\"' = 'id="{0}-aspnetruntime" version="{1}"' -f (Get-DotNetPackagePrefix -Version $Latest.ReleaseVersion -IncludeMajorMinor), (Get-DotNetDependencyVersionRange -BaseVersion $Latest.Version -Boundary 'Patch')
        }
    }
}
...
function global:au_GetLatest {
    ...
    foreach ($channel in (Get-DotNetChannels))
    {
        ...
        $infosForChannel = Get-DotNetRuntimeComponentUpdateInfo -Channel $channel -Component 'AspNetRuntime' -AllVersions:$AllVersionsAsStreams
